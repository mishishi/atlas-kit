#!/usr/bin/env node
/**
 * R64 clean — handwrite descriptions + taglines + fallback history
 * for 25 R64 cards. Same append-or-patch pattern as r62/r63.
 */
...
const PLAN = path.join(ROOT, "tmp", "plan-r64.json");
...
const COPY = {
  "calcium-elem": {
    desc: "钙 是化学符号 Ca、原子序数 20 的碱土金属, 地壳中含量第 5 高的元素 (4.1% 质量)。人体内 99% 的钙在骨骼和牙齿中, 1% 在血液 (维持肌肉收缩和神经信号), 是人体第二丰富的金属元素 (仅次于铁)。日常需求 1000mg/天, 牛奶豆腐深绿色蔬菜是天然来源。",
    tagline: "骨骼牙齿的主元素, 日常 1000mg",
  },
  "strontium": {
    desc: "锶 是化学符号 Sr、原子序数 38 的碱土金属, 1787 年英国矿物学家阿德罗因德在苏格兰斯特朗廷 (Strontian) 村发现, 元素名即来源地名。最显著用途是彩色电视显像管玻璃 (防 X 射线泄漏) 和烟火红色烟火 (锶盐)。放射性 90-Sr 是 1986 年切尔诺贝利事故污染最重的同位素之一。",
    tagline: "1787 苏格兰发现的碱土金属",
  },
  "vancouver": {
    desc: "温哥华 加拿大不列颠哥伦比亚省大都会区, 2010-2024 年间连续 8 次被美世咨询 (Mercer) 评为'世界最宜居城市'。人口 250 万, 温带海洋性气候, 冬天 0°C + 多雨, 是华人比例最高 (30%) 的北美城市之一。",
    tagline: "8 次蝉联世界最宜居城市",
  },
  "shantou": {
    desc: "汕头 是中国广东省东部粤东地区的核心城市, 1861 年被辟为'五口通商'之一, 是中国最早对外开放的港口之一。'潮汕文化'以精细的潮州工夫茶、潮州话 (保留古汉语 8 个声调) 著名于世, 人口 540 万, 海外潮汕籍华侨 1000 万。",
    tagline: "1861 五口通商之一, 潮汕文化中心",
  },
  "jiaxing": {
  ...
  "tsunami-cnt": {
    desc: "2004 年 12 月 26 日印度洋海啸 是 21 世纪死亡人数最多的自然灾害, 苏门答腊岛西海岸 9.1-9.3 级地震引发越洋海啸。袭击印度尼西亚、泰国、斯里兰卡、印度 14 国海岸, 死亡 22.7 万人, 170 万人无家可归。事件直接推动 2005 年 1 月联合国'全球海啸预警系统'启动, 是世界级灾害预警机制的核心催化剂。",
    tagline: "2004.12.26 死亡 22.7 万, 催生预警系统",
  },
  "kuroshio-current": {
    desc: "黑潮 (Kuroshio Current) 是西太平洋最强的暖流, 流速 2 节 (3.7 km/h), 宽度 100 公里, 自菲律宾以东沿台湾东海岸、冲绳、九州、四国南岸到日本本州南岸进入太平洋, 年输送热量 = 100 万个核反应堆, 是地球大气 - 海洋热量交换的最大引擎之一。 与北大西洋湾流 (Gulf Stream) 并称'世界两大暖流'。",
    tagline: "世界两大暖流之一, 100 万核反应堆能量",
  },
};
...
for (const [slug, copy] of Object.entries(COPY)) {
  ...
  if (!existing) {
    // R64 (2026-07-01): matrix gave 19/25 'no output_url in matrix response'
    // errors this run. The 6 successful cards already exist in cards.json
    // (from generate-card.mjs); the 19 failed cards did NOT get added.
    // To avoid shipping empty-image entries, skip the failed slugs here.
    skippedFailed++;
    continue;
  }
...
console.log(`R64 clean: patched ${updated} cards; skipped ${skippedFailed} matrix-failed slugs (no image).`);