...
第一张是横构图, 第二张变竖构图, 第三张又把主角放右下角。我想做的是「同一系列的视觉一致」, 这样几张图鉴并排放, 看起来是一套东西。但 AI 工具默认每张图都是独立艺术品, 没人帮你锁定版式。
...
AI 生成的「示意图」我本来画了 2 张, 写完发现太简陋, 删了。下面是项目里 4 张真实卡片的缩略图 (直接链 atlas-kit 站点的 -thumb.webp, 跟产品页用的同源), 分别代表 4 个系列——
...
![拉布拉多 — 宠物品种图鉴](https://atlas-kit-six.vercel.app/cards/labrador-retriever-thumb.webp)
**拉布拉多** · 适配评分 9 分 · 解释: "全场景家庭犬, 比金毛掉毛少, 适合公寓。" (项目里这张其实 slug 叫 labrador-retriever, 我填主题的时候写成了「金毛」, 后来懒得改数据, 缩略图沿用了原始生成的拉布拉多。这是个小数据 bug, 但每次看到我都会笑, 留着。)

![故宫 — 城市百科图鉴](https://atlas-kit-six.vercel.app/cards/forbidden-city-thumb.webp)
**故宫 (北京)** · 适配评分 — 字段不适用 · 城市卡片的「适配评分」我直接删了, 因为没人会问「我适不适合住在故宫」。

![苏州园林 — 城市百科图鉴](https://atlas-kit-six.vercel.app/cards/suzhou-gardens-thumb.webp)
**苏州园林** · 适配评分 8 分 · 解释: "江南城市里文化密度最高的一座, 慢节奏、园林细节多, 适合喜欢步行探索的人。" 这里 8 分不是「好玩度」, 是「居住适配度」——读者可能误读, 但我懒得加 legend。

![三星堆 — 图鉴杂俎](https://atlas-kit-six.vercel.app/cards/sanxingdui-thumb.webp)
**三星堆** · 适配评分 — 字段不存在 · 青铜面具没有「适不适合养」这件事。**字段会随主题消失, 这不是 bug, 是设计。**
...
金毛那张 (其实就是上面那张拉布拉多, 同一个 bug), 适配评分我给 8 分, 解释是"对新手友好但掉毛量惊人"。
...
图鉴之间的关联, 一开始我只做了"金毛 → 拉布拉多"这种同类推荐。后来发现更值钱的关联是"拉布拉多 ← 哪些图鉴提到了拉布拉多"——比如有一张金毛图鉴里讲"和拉布拉多很像但更适合小户型", 这条就把两张卡串起来了。
